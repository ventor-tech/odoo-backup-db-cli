...
def print_error_dir(dir_path):
    """Print details about error dir."""
    logging.info('Creation of the directory "{0}" failed.'.format(dir_path))
    logging.info(
        'Please change access rights or owner for parent directory and try again.'
    )
    logging.info(
        'If you want change access rights, probably next command help you.')
...
def dump_db(config, environment):
    ...
    db_config = (
        config[environment].get(
            'db_username', config[DEFAULT_ENVIRONMENT].get('db_username')),
        config[environment].get(
            'db_password', config[DEFAULT_ENVIRONMENT].get('db_password')),
        config[environment].get('db_host',
                                config[DEFAULT_ENVIRONMENT].get('db_host')),
        config[environment].get('db_port',
                                config[DEFAULT_ENVIRONMENT].get('db_port')),
...
def check_config(config, environment):  # noqa: C901, WPS212, WPS210, WPS213, WPS231
    ...
    if clean_backup_after is None:
        logging.info(
            'The settings do not indicate after how many days to delete.')
        return CodeError.NO_SETTINGS
    if config[environment].get('with_filestore') not in ('False', '0', None):
    ...
    if type not in TYPES:
        ...
        if auth_types.issubset(all_config_fields):
            logging.info(
                'Only one of these fields must be present in settings environment: {0}'
                .format(auth_types))
